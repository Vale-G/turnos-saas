  ...
  const navItems = esStaff
    ...
    : [
        ...
        },
        {
          ...
          href: esPro || esTrial || esSuperAdmin ? '/caja' : '#',
          badge: esPro || esTrial || esSuperAdmin ? null : 'PRO',
          proOnly: true,
        },
        {
          label: 'Informes',
          desc: 'Estadísticas PRO',
          href: esPro || esTrial || esSuperAdmin ? '/informes' : '#',
          badge: esPro || esTrial || esSuperAdmin ? null : 'PRO',
          proOnly: true,
        },
        {
        ...
        },
        {
          label: 'Servicios',
          desc:
            esPro || esTrial || esSuperAdmin
              ? 'Sin límite'
              : serviciosCount + ' / ' + limites.maxServicios,
          href: '/servicios',
          badge:
            !esPro &&
            !esTrial &&
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-6">
            {negocio?.logo_url ? (
              <Image
                ...
                height={80}
                unoptimized
                className="w-20 h-20 object-contain rounded-[2rem] bg-white/5 p-2 border border-white/10 shadow-xl"
              />
            ) : (
              <div
                className="w-20 h-20 bg-white/5 rounded-[2rem] border border-white/10 flex items-center justify-center text-4xl font-black shadow-xl"
            ...
            <div>
              <div className="flex items-center gap-4 mb-2">
                ...
                {esSuperAdmin ? (
                  <span className="bg-rose-500 text-black text-[10px] font-black uppercase px-3 py-1 rounded-full shadow-lg shadow-rose-500/20">
                    DIOS
                  </span>
                ) : esStaff ? (
                  <span className="bg-slate-500/20 text-slate-400 text-[10px] font-black uppercase px-3 py-1 rounded-full">
                    STAFF
                  </span>
                ) : esPro ? (
                  ...
                  <span className="bg-blue-500 text-white text-[10px] font-black uppercase px-3 py-1 rounded-full">
                    TRIAL · {diasTrial}D
                  </span>
                ) : (
                  <span className="bg-white/10 text-slate-400 text-[10px] font-black uppercase px-3 py-1 rounded-full">
                    NORMAL
                  </span>
                )}
              </div>
          ...
          <div className="flex gap-3">
            {esSuperAdmin && (
              <button
                ...
                className="text-[10px] font-black uppercase tracking-widest text-rose-400 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 transition-colors px-6 py-3 rounded-2xl"
              >
                Modo Dios
              </button>
            )}
            <button
              ...
              className="text-[10px] font-black uppercase tracking-widest text-slate-600 hover:text-white transition-colors bg-white/5 px-6 py-3 rounded-2xl"
            >
              Cerrar Sesión
            </button>
          </div>
        ...
        {esTrial && diasTrial <= 7 && !esStaff && !esSuperAdmin && (
          <div className="mb-10 p-8 rounded-[3rem] border border-blue-500/30 bg-blue-500/10 flex flex-col md:flex-row items-center justify-between gap-6 cursor-pointer hover:bg-blue-500/15 transition-all">
            <div>
              <p className="font-black uppercase text-xl text-blue-400 mb-1 tracking-tight">
                TRIAL TERMINA EN {diasTrial} DÍA{diasTrial !== 1 ? 'S' : ''}
              </p>
              <p className="text-blue-200/60 text-xs font-bold uppercase tracking-widest">
                Activá tu plan PRO para no perder acceso al sistema.
              </p>
            </div>
            <button
              onClick={() =>
                window.open(
                  'https://wa.me/5491123456789?text=Hola,%20quiero%20pasar%20a%20PRO',
                  '_blank'
                )
              }
              className="text-[10px] font-black uppercase text-black bg-blue-400 px-6 py-3 rounded-2xl shadow-lg shadow-blue-500/20"
            >
              Contactar
            </button>
          </div>
        ...
        <div
          className={`grid grid-cols-1 md:grid-cols-2 ${esStaff ? 'lg:grid-cols-2' : 'lg:grid-cols-4'} gap-5`}
        >
          {navItems.map((item) => (
            <button
              key={item.label}
              onClick={() => router.push(item.href)}
              className="bg-white/4 p-10 rounded-[3.5rem] border border-white/5 hover:border-white/20 transition-all text-left group relative overflow-hidden active:scale-95 shadow-lg"
            >
              {item.badge && (
                <span className="absolute top-8 right-8 bg-rose-500/20 text-rose-400 border border-rose-500/30 text-[9px] font-black uppercase px-3 py-1 rounded-full">
                  {item.badge}
                </span>
              )}
              <div className="text-5xl mb-6 opacity-40 group-hover:opacity-100 transition-all group-hover:scale-110 origin-left">
                {ICONS[item.label] || '✨'}
              </div>
              <h3
        ...
        {!esStaff && (
          <div
            className="mt-12 p-10 rounded-[3.5rem] border flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl relative overflow-hidden group"
            style={{
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/50 text-[10px] font-black uppercase tracking-[0.3em]">
                Copiá esto en tu biografía de Instagram
              </p>
            </div>
            <button
              onClick={() => {
            ...
            >
              <span style={{ color: colorPrincipal }}>
                {'/reservar/' + negocio?.slug}
              </span>
              <span
                className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl transition-all ${copiado ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/10 text-white'}`}
              >
