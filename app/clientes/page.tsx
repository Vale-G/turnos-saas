...
export default function Clientes() {
  ...
  const guardarNota = useCallback(async (clienteId: string, clienteNombre: string) => {
    ...
    await supabase.from('clientenota').insert({
      negocio_id: negocioId, cliente_id: clienteId,
      cliente_nombre: clienteNombre, nota: nota.trim(),
    })
    setNota('')
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtrados.map(c => (
              <div key={c.cliente_id} className="bg-white/4 border border-white/8 hover:border-white/15 rounded-2xl overflow-hidden transition-all">
                <div className="p-5 flex items-center justify-between gap-4" onClick={() => setClienteAbierto(clienteAbierto === c.cliente_id ? null : c.cliente_id)} style={{ cursor: 'pointer' }}>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center font-black text-base flex-shrink-0" style={{ background: colorPrincipal + '20', color: colorPrincipal }}>{c.cliente_nombre[0]?.toUpperCase()}</div>
                    <div>
                      <p className="font-black uppercase text-sm">{c.cliente_nombre}</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">{c.servicioFavorito && 'Favorito: ' + c.servicioFavorito + ' · '}Última visita: {c.ultimaVisita}</p>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {c.turnos.map(t => (
                        <div key={t.id} className="flex items-center justify-between bg-white/4 border border-white/8 rounded-xl px-4 py-2.5">
                          <div><p className="text-xs font-black">{t.servicio?.nombre ?? 'Servicio'}</p><p className="text-[10px] text-slate-500">{t.fecha} · {t.hora?.slice(0, 5)} · {t.staff?.nombre ?? ''}</p></div>
                          <div className="text-right">
                            <p className="text-[10px] font-black" style={{ color: colorPrincipal }}>{t.servicio?.precio ? '$' + t.servicio.precio : ''}</p>
                            <p className={'text-[9px] font-black ' + (t.estado === 'completado' ? 'text-slate-400' : t.estado === 'cancelado' ? 'text-red-400' : 'text-amber-400')}>{t.estado}</p>
                          </div>
                        </div>
                      ))}
                    </div>
