...
export default function InformesElite() {
  ...
  const stats = useMemo(() => {
    ...
    // Mapeo de ingresos por día para el gráfico
    ...
    <div className="min-h-screen bg-[#020617] text-white p-6 md:p-12">
      <div className="max-w-5xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-600 mb-4">Métricas de Negocio</p>
          ...
          <div className="flex bg-white/5 border border-white/10 p-1.5 rounded-2xl backdrop-blur-md">
            {(['7d', '30d', '90d'] as const).map(p => (
              <button key={p} onClick={() => setPeriodo(p)} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${periodo === p ? 'text-black' : 'text-slate-500'}`} style={periodo === p ? {backgroundColor: colorP} : {}}>{p === '7d' ? 'Semana' : p === '30d' ? 'Mes' : 'Trimestre'}</button>
            ))}
          </div>
        </header>

        {loading ? (
           <div className="text-center py-40 font-black italic text-slate-800 animate-pulse text-4xl">ANALIZANDO...</div>
        ) : (
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white/5 border border-white/10 p-10 rounded-[3rem]">
                <p className="text-[10px] font-black uppercase text-slate-500 mb-4 tracking-widest">Turnos Activos</p>
                <p className="text-5xl font-black italic">{stats.cantidad}</p>
              </div>
              <div className="bg-white/5 border border-white/10 p-10 rounded-[3rem]">
                <p className="text-[10px] font-black uppercase text-slate-500 mb-4 tracking-widest">Ticket Promedio</p>
                <p className="text-5xl font-black italic" style={{color: colorP}}>${Math.round(stats.ticket).toLocaleString()}</p>
              </div>
            </div>

            <div className="bg-white/5 border border-white/10 p-10 rounded-[3rem]">
              <p className="text-[10px] font-black uppercase text-slate-500 mb-10 tracking-widest text-center">Rendimiento Diario (Ingresos)</p>
              <div className="flex items-end gap-2 h-40">
                {stats.chartData.length === 0 ? <p className="w-full text-center text-slate-700 italic font-black">Sin datos para este periodo</p> : stats.chartData.map(([f, m]) => (
