  ...
  useEffect(() => {
    async function init() {
      ...
      if (!negId) {
         ...
         if (n) {
           negId = n.id
           setColorP(getThemeColor(n.tema))
           if (n.suscripcion_tipo === 'pro' || n.suscripcion_tipo === 'trial') setTieneAcceso(true)
         }
      } else {
        ...
        if (neg) {
          ...
          if (neg.suscripcion_tipo === 'pro' || neg.suscripcion_tipo === 'trial') setTieneAcceso(true)
        }
      }
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white p-6 md:p-12">
      <div className="max-w-5xl mx-auto text-center py-20 animate-in fade-in zoom-in-95">
        ...
        <p className="text-slate-400 text-sm max-w-lg mx-auto bg-white/5 p-6 rounded-2xl border border-white/10">Métricas avanzadas, ticket promedio y rendimiento por staff próximamente en este módulo. Estamos trabajando en ello 🚀</p>
      </div>
    </div>
  )
}
