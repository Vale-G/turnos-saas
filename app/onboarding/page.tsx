...
export default function OnboardingElite() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] blur-[150px] opacity-10 transition-colors duration-1000 pointer-events-none"
      ...
      <div className="w-full max-w-xl relative z-10">
        <div className="text-center mb-12">
          ...
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.4em]">
            Configuración Inicial · Paso {paso} de 3
          </p>
          <div className="flex gap-2 mt-6 justify-center">
            <div
              className={`h-1.5 w-12 rounded-full transition-all duration-500 ${paso >= 1 ? 'bg-white' : 'bg-white/10'}`}
            />
            <div
              className={`h-1.5 w-12 rounded-full transition-all duration-500 ${paso >= 2 ? 'bg-white' : 'bg-white/10'}`}
            />
            <div
              className={`h-1.5 w-12 rounded-full transition-all duration-500 ${paso >= 3 ? 'bg-white' : 'bg-white/10'}`}
            />
          </div>
        </div>
        ...
        <form
          ...
          className="bg-white/5 border border-white/10 p-10 md:p-12 rounded-[3.5rem] backdrop-blur-md shadow-2xl relative overflow-hidden min-h-[400px] flex flex-col justify-between"
        >
          {paso === 1 && (
            <div className="animate-in slide-in-from-right-8 duration-500 space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="EJ: ESTÉTICA GLAM"
                  required
                  className="w-full bg-black/50 border border-white/10 p-6 rounded-2xl text-sm font-black uppercase outline-none focus:border-white/30 transition-all placeholder:text-slate-800"
                />
              </div>
              <div className="pt-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 block">
                  Link Único de Reservas
                </label>
                <div className="flex items-center bg-black/50 border border-white/10 rounded-2xl overflow-hidden focus-within:border-white/30 transition-all p-2">
                  <span className="pl-4 pr-2 text-xs font-black text-slate-600">
                    /reservar/
                  </span>
                  <input
                    value={slug}
                    ...
                    required
                    className="w-full bg-transparent py-4 pr-4 text-xs font-black outline-none text-white"
                  />
                </div>
              </div>
            </div>
          ...
          {paso === 2 && (
            <div className="animate-in slide-in-from-right-8 duration-500 space-y-6">
              <button
                type="button"
                onClick={() => setPaso(1)}
                className="text-[10px] font-black uppercase tracking-widest text-slate-600 hover:text-white mb-4 block"
              >
                ← Volver
              </button>
              <h2 className="text-2xl font-black uppercase italic tracking-tighter mb-8">
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderColor: tema === key ? 'white' : 'transparent',
                          boxShadow:
                            tema === key ? `0 0 30px ${colorHex}80` : 'none',
                        }}
                      />
                    )
                  })}
          ...
          <button
            type="submit"
            disabled={guardando || (paso === 1 && !nombre)}
            className={`w-full py-6 rounded-[2.5rem] font-black uppercase italic text-lg transition-all mt-10 shadow-2xl active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${paso === 3 ? 'text-black' : 'bg-white text-black'}`}
            style={paso === 3 ? { backgroundColor: colorP } : {}}
          >
            {guardando
              ? 'CREANDO...'
              : paso === 3
