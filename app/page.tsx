...
export default function Home() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center gap-6 text-center p-4">
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 flex font-sans selection:bg-[#10b981]/30">
      ...
      <aside className="w-80 border-r border-white/5 bg-[#020617] flex flex-col p-10 gap-10 sticky top-0 h-screen z-50 overflow-y-auto">
        ...
        {msg.t && <div className={`p-4 rounded-2xl text-[10px] font-black uppercase text-center animate-bounce ${msg.tipo === 'success' ? 'bg-[#10b981]/20 text-[#10b981]' : 'bg-red-500/20 text-red-500'}`}>{msg.t}</div>}

        <div className="bg-[#0f172a] p-6 rounded-[2.5rem] border border-white/5">
          <div className="grid grid-cols-2 gap-2">
            {['admin', 'peluquero', 'cliente', 'superadmin'].map(r => (
              <button key={r} onClick={() => { setRol(r as any); setSeccionActiva('agenda'); }} className={`py-2 rounded-xl text-[7px] font-black uppercase transition-all ${rol === r ? 'bg-white text-black' : 'text-slate-600'}`}>{r}</button>
            ))}
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="p-12 border-b border-white/5 flex justify-between items-center bg-[#020617]/80 backdrop-blur-2xl sticky top-0 z-40">
          <div>
            ...
            <p className="text-[10px] text-[#10b981] font-black uppercase tracking-[0.3em] mt-3">{rol} mode</p>
          </div>
          <select value={negocioActual.id} onChange={(e) => setNegocioActual(negocios.find(n => n.id === e.target.value))} className="bg-[#0f172a] border border-white/10 text-white text-[10px] font-black p-5 rounded-2xl outline-none uppercase">
        ...
        <div className="p-12 max-w-7xl mx-auto space-y-16 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              <div className="lg:col-span-8 space-y-10">
                <div className="flex justify-between items-center border-b border-white/5 pb-8">
                  <h3 className="text-3xl font-black text-white uppercase italic tracking-tighter">Agenda Operativa</h3>
                  <input type="date" value={filtroFecha} onChange={e => setFiltroFecha(e.target.value)} className="bg-[#0f172a] border border-white/10 p-4 rounded-xl text-[#10b981] font-black text-xs outline-none" />
                </div>
                  ...
                  {turnosHoy.map((t: any) => (
                    <div key={t.id} className={`p-8 rounded-[3.5rem] border flex justify-between items-center group transition-all ${t.estado === 'proceso' ? 'bg-[#10b981]/10 border-[#10b981]' : 'bg-[#0f172a] border-white/5'}`}>
                      <div className="flex items-center gap-8">
                        <div className="w-20 h-20 bg-[#020617] rounded-[1.8rem] flex items-center justify-center border border-white/10 font-black text-2xl text-[#10b981]">{new Date(t.hora_inicio).getHours()}:00</div>
                        <div>
                      ...
                      {['admin', 'peluquero', 'superadmin'].includes(rol) && t.estado !== 'finalizado' && (
                        ...
                        }} className="bg-white text-black px-10 py-4 rounded-[1.5rem] font-black text-[10px] uppercase hover:bg-[#10b981] transition-all">{t.estado === 'pendiente' ? 'Iniciar' : 'Cobrar'}</button>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="lg:col-span-4 space-y-8">
                {rol !== 'superadmin' && (
                  <section className="bg-gradient-to-br from-[#0f172a] to-[#020617] p-10 rounded-[3.5rem] border border-white/10 shadow-2xl">
                    <h4 className="text-white font-black uppercase italic mb-8 tracking-tighter text-xl">Nueva Reserva</h4>
                    <form onSubmit={handleCrearTurno} className="space-y-5">
                      ...
                      <button className="w-full bg-[#10b981] text-black font-black py-5 rounded-[2rem] uppercase text-[10px] tracking-widest shadow-xl">Confirmar</button>
                    </form>
                  </section>
                )}
          ...
          {seccionActiva === 'clientes' && (
            <div className="space-y-12 animate-in fade-in">
              <h3 className="text-5xl font-black text-white italic uppercase tracking-tighter">Ranking <span className="text-[#10b981]">CRM</span></h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {getTopClientes().map(([n, d]: any) => (
                  <div key={n} className="bg-[#0f172a] p-10 rounded-[4rem] border border-white/5">
                    ...
                    <div className="grid grid-cols-2 gap-6 mt-10 pt-8 border-t border-white/5">
                      <div><p className="text-[10px] text-slate-500 uppercase">Visitas</p><p className="text-2xl font-black text-[#10b981] italic">{d.v}</p></div>
                      <div><p className="text-[10px] text-slate-500 uppercase">Total</p><p className="text-2xl font-black text-white italic">${d.s}</p></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {seccionActiva === 'servicios' && (
            <div className="space-y-12">
              <h3 className="text-5xl font-black text-white italic uppercase tracking-tighter">Catálogo</h3>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                  {negocioActual.Servicio?.map((s: any) => (
                    <div key={s.id} className="bg-[#0f172a] p-12 rounded-[4rem] border border-white/5 group">
                      <p className="text-white font-black uppercase italic text-3xl tracking-tighter">{s.nombre}</p>
                      <p className="text-[#10b981] font-black text-5xl mt-6 italic">${s.precio}</p>
                    </div>
                  ))}
                </div>
                {rol === 'admin' && (
                  <div className="lg:col-span-4 bg-[#020617] border border-[#10b981]/20 p-10 rounded-[3.5rem]">
                    ...
                    <form onSubmit={handleCrearServicio} className="space-y-4">
                      ...
                      <button className="w-full bg-[#10b981] text-black font-black py-5 rounded-2xl uppercase text-[11px]">Publicar</button>
                    </form>
                  </div>
                )}
              </div>
          ...
          {seccionActiva === 'staff' && (
            <div className="space-y-12">
              <h3 className="text-5xl font-black text-white italic uppercase tracking-tighter">Equipo</h3>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                  {negocioActual.Staff?.map((s: any) => (
                    <div key={s.id} className="bg-[#0f172a] p-12 rounded-[4rem] border border-white/5 text-center transition-all hover:border-[#10b981]/30">
                      <div className="w-24 h-24 bg-gradient-to-br from-[#10b981]/20 to-transparent rounded-full flex items-center justify-center text-4xl mx-auto mb-8 shadow-2xl">👤</div>
                ...
                {rol === 'admin' && (
                  <div className="lg:col-span-4 bg-[#020617] border border-[#10b981]/20 p-10 rounded-[3.5rem] h-fit">
                    ...
                    <form onSubmit={handleCrearStaff} className="space-y-4">
                      ...
                      <button className="w-full bg-[#10b981] text-black font-black py-5 rounded-2xl uppercase text-[10px]">Activar</button>
                    </form>
                  </div>
                )}
              </div>
          ...
          {seccionActiva === 'finanzas' && (
            <div className="space-y-12 animate-in zoom-in-95">
              <h3 className="text-5xl font-black text-white italic uppercase tracking-tighter">BI <span className="text-[#10b981]">Stats</span></h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="bg-[#0f172a] p-12 rounded-[4rem] border border-white/5 text-center">
                  <p className="text-[10px] font-black text-slate-500 uppercase mb-4">Ocupación</p>
                  <p className="text-6xl font-black text-white italic tracking-tighter">84%</p>
                </div>
                <div className="bg-[#10b981] p-12 rounded-[4rem] text-[#020617]">
                  <p className="text-[10px] font-black uppercase mb-4">Top Servicio</p>
                  <p className="text-3xl font-black italic tracking-tighter uppercase">{negocioActual.Servicio?.[0]?.nombre || '---'}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {seccionActiva === 'admin' && rol === 'superadmin' && (
            <div className="space-y-12">
               <h3 className="text-6xl font-black text-white italic uppercase tracking-tighter">Master <span className="text-[#10b981]">Control</span></h3>
               <div className="bg-[#0f172a] p-16 rounded-[5rem] border border-[#10b981]/20">
                 <p className="text-[12px] font-black text-[#10b981] uppercase mb-10 text-center tracking-[0.4em]">Registrar Nueva Entidad</p>
                 <form onSubmit={handleCrearNegocio} className="flex gap-6 max-w-4xl mx-auto">
                   <input type="text" value={fNegocio.nombre} onChange={e => setFNegocio({nombre: e.target.value})} placeholder="Nombre del local" className="flex-1 bg-[#020617] border border-white/10 p-7 rounded-[2rem] text-lg text-white font-black outline-none" />
                   <button className="bg-white text-black px-12 rounded-[2rem] font-black uppercase text-xs hover:bg-[#10b981] transition-all">Desplegar</button>
                 </form>
               </div>
            </div>
          )}
