...
export default function ReservaPublica() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (neg) {
        ...
        const inicio = parseInt(neg.hora_inicio?.split(':')[0] || '9')
        const fin = parseInt(neg.hora_fin?.split(':')[0] || '20')
        const slots = []
  ...
  if (confirmado) return (
    <div className="min-h-screen bg-[#020617] text-white flex flex-col items-center justify-center p-6 text-center">
      ...
      <p className="text-slate-500 mt-2 font-bold uppercase text-[10px]">Agendado para las {hora} hs</p>
    </div>
  )
      ...
      <div className="max-w-md mx-auto space-y-8 py-10">
        ...
        {paso === 2 && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2">
              {horarios.map(h => (
                <button key={h} onClick={() => { setHora(h); setPaso(3); }} className="bg-slate-900 p-4 rounded-2xl border border-slate-800 font-bold active:bg-emerald-500 active:text-black transition-all">
                  {h}
                </button>
              ))}
            </div>
        ...
        {paso === 3 && (
          <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 space-y-6 shadow-2xl">
            <div className="text-center"><p className="text-emerald-500 font-black text-2xl italic">{hora} HS</p></div>
