...
export default function GestionStaff() {
  ...
  if (loading)
    return (
      <div
        className="min-h-screen bg-[#020617] flex items-center justify-center font-black italic animate-pulse"
        ...
        {lleno && plan === 'normal' ? (
          <div className="bg-amber-400/10 border border-amber-400/20 rounded-2xl p-5 mb-8">
            <p className="font-black text-amber-400 text-sm uppercase">
              Límite del plan Normal
            </p>
            <p className="text-slate-400 text-xs mt-0.5">
              Tenés {limite} barberos. Upgrade a Pro para agregar más.
            </p>
          </div>
        ) : (
          <form
            onSubmit={agregarBarbero}
          ...
          {staff.map((persona) => (
            ...
            >
              <div>
                <h3
                  className={
                    'font-black uppercase italic text-xl tracking-tight ' +
                    (!persona.activo && 'text-slate-600 line-through')
                  }
                >
                  {persona.nombre}
                </h3>
                <p
                  className={
                    ...
                    (persona.activo ? 'text-emerald-500' : 'text-red-400')
                  }
                >
