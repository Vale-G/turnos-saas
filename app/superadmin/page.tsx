...
export default function SuperAdmin() {
  ...
  async function cargarNegocios() {
    setLoading(true)
    const { data, error } = await supabase.from('Negocio').select('*').order('created_at', { ascending: false })
  ...
  async function eliminarNegocio(id: string, nombre: string) {
    if (!confirm('Seguro que queres eliminar "' + nombre + '"? Esta accion no se puede deshacer.')) return
          ...
          {negociosFiltrados.length === 0 ? (
            ...
            >
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  onClick={() => window.open('/reservar/' + n.slug, '_blank')}
