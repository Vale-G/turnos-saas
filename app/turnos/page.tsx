...
export default function AgendaTurnos() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // 1. Buscamos si es empleado (Staff)
      const { data: adm } = await supabase.from('adminrol').select('*').eq('user_id', user.id).maybeSingle()
      if (adm?.negocio_id) {
        nId = adm.negocio_id
      } else {
        // 2. Si no es empleado, buscamos si es dueño
        const { data: n } = await supabase.from('negocio').select('id, tema').eq('owner_id', user.id).order('created_at', { ascending: false }).limit(1).single()
        if (n) {
           nId = n.id
           negData = n
        }
      ...
      if (!nId) { router.push('/onboarding'); return }
      
      // 3. Cargamos los datos del negocio si somos staff (porque no los trajimos en el paso 2)
      if (!negData) {
