      ...
      <div className="bg-[#0F172A] rounded-[2rem] border border-white/8 p-6">
        ...
        <div className="space-y-2">
          {TURNOS_DEMO.map((turno) => (
            <div key={turno.nombre} className="bg-[#1E293B] rounded-xl p-3 flex items-center gap-3">
              <div className="w-1 h-10 rounded-full flex-shrink-0" style={{ backgroundColor: turno.color }} />
              <div className="flex-1 min-w-0">
                <p className="font-black text-sm truncate">{turno.nombre}</p>
                <p className="text-[10px] text-slate-500 truncate">{turno.servicio}</p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="font-black text-sm" style={{ color: turno.color }}>
                  {turno.hora}
                </p>
                <p className="text-[9px] uppercase font-black" style={{ color: turno.color }}>
                  {turno.estado}
                </p>
              </div>
            </div>
